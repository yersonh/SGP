 ...
 * @return array Lista de referenciados
 */
public function getReferenciadosPaginados($page = 1, $perPage = 50, $filters = []) {
    ...
    // ============================================
    // BÚSQUEDA GLOBAL - UNIFICADA PARA TEXTO Y NÚMEROS
    // ============================================
    if (!empty($filters['search'])) {
        $search = $filters['search'];
        
        // Lista completa de campos para búsqueda
        $conditions[] = "(
            -- Campos principales de referenciados
            r.nombre ILIKE ? OR 
            r.apellido ILIKE ? OR 
            r.cedula ILIKE ? OR 
            ...
            -- Número de mesa (convertido a texto)
            CAST(r.mesa AS TEXT) ILIKE ?
        )";
        
        // Mismo término para todos los campos
        $searchTerm = '%' . $search . '%';
        
        // Total de campos: 20 (contar los signos ? en la condición)
        for ($i = 0; $i < 20; $i++) {
            $params[] = $searchTerm;
    ...
    // ============================================
    // FILTRO ESPECÍFICO POR NÚMERO DE MESA
    // ============================================
...
public function getTotalReferenciados($filters = []) {
    ...
    if (!empty($filters['search'])) {
        ...
        // Misma lista de campos que en getReferenciadosPaginados
        $conditions[] = "(
            r.nombre ILIKE ? OR 
            r.apellido ILIKE ? OR 
            r.cedula ILIKE ? OR 
        ...
        // 20 parámetros (contar los signos ?)
        for ($i = 0; $i < 20; $i++) {
            $params[] = $searchTerm;
            $paramTypes[] = \PDO::PARAM_STR;
        }
