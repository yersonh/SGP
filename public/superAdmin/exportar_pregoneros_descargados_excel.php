...
header('Content-Disposition: attachment; filename="pregoneros_descargados_' . date('Y-m-d_H-i-s') . '.xls"');
...
echo '<x:Name>Pregoneros Descargados</x:Name>';
...
echo '.header { background-color: #27ae60; color: white; font-weight: bold; }';
...
// Mostrar filtro principal (siempre aplicado)
echo '<tr><td><strong>Filtro principal:</strong></td><td>Pregoneros que ya votaron</td></tr>';
...
foreach ($pregoneros as $pregonero) {
    ...
    // Determinar voto registrado (siempre será SI en esta vista)
...
echo '<tr><th colspan="2" class="header">RESUMEN DE PREGONEROS DESCARGADOS</th></tr>';
echo '<tr><td><strong>Total Pregoneros que votaron:</strong></td><td>' . count($pregoneros) . '</td></tr>';
...
if (!empty($referenciadoresCount)) {
    echo '<tr><td colspan="2" style="background-color: #e9ecef; font-weight: bold; text-align: center;">Top 5 Referenciadores con más Pregoneros Descargados</td></tr>';
...
echo '<tr><td><strong>Tipo de Exportación:</strong></td><td>Pregoneros que ya votaron' . (empty($filtros) ? '' : ' con filtros adicionales') . '</td></tr>';
echo '</table>';
